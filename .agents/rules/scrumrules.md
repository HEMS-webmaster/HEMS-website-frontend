...
## 6. Data Architecture Integrity (Workshop Ingestion)
1. **The Source of Truth Lock**: When asked to "autocomplete," ingest, or parse new workshop data, **@dev** must ALWAYS write the output directly into the corresponding workshop object within `src/frontend/src/data/master_workshops.json`.
2. **Volatile File Prohibition**: **@dev** is strictly prohibited from manually editing any individual year files within `src/frontend/src/data/archives/` (e.g., `2015.json`). These files are volatile outputs generated by the Workshop Manager's compiler.
3. **Contradiction Protocol**: If **@bo** explicitly requests a manual edit to a file inside the `archives/` directory, **@dev** must immediately halt, cite the "Data Architecture Integrity" rule, explain that the edit will be wiped out upon the next compile, and await explicit bypass permission from **@bo** before proceeding.


## 7. The Chain of Command & Deployment Gates
...
## 8. The Silent Chain-of-Thought (SCoT) Protocol
...
## 9. The "Not My Job" (Misroute) Protocol
