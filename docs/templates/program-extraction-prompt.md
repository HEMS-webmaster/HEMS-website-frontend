# HEMS Workshop Program Extraction Prompt

> **Purpose:** Give this prompt to an AI agent along with four URLs — a **Workshop Overview** page, a **Technical Program** page, a **Quick Links / Student Awards** page, and a **Corporate Sponsors** page. The agent will read the content of all four pages and produce a structured markdown file containing all extractable data, formatted to be directly loaded into the HEMS Workshop Manager.
...
**Target Workshop Number:** [INSERT_NUMBER, e.g. 14]
**Target Workshop Year:** [INSERT_YEAR, e.g. 2022]

**Workshop Overview URL:** [INSERT_OVERVIEW_URL, e.g. https://www.hems-workshop.org/14thWS/14thWS.html]
**Technical Program URL:** [INSERT_PROGRAM_URL, e.g. https://www.hems-workshop.org/14thWS/14thProgram.html]
**Quick Links / Student Awards URL:** [INSERT_QUICKLINKS_URL, e.g. https://www.hems-workshop.org/14thWS/quicklinkalpha.html]
**Corporate Sponsors URL:** [INSERT_SPONSORS_URL, e.g. https://www.hems-workshop.org/14thWS/14thWS.html#sponsors]

Read the text content of ALL FOUR URLs above. Do NOT follow or open any links found within those pages.
...
## Post-Extraction Checklist (For the Human Operator)

After the agent produces the markdown, verify these items before loading into the Workshop Manager:

- [ ] **Workshop metadata** matches the target year and number.
- [ ] **All session dates** are in ISO format and correctly mapped to the workshop year.
- [ ] **Presenter markers** `[PRESENTER]` are present for every talk (one per talk).
- [ ] **Author names** are not truncated or merged.
- [ ] **Legacy URLs** are raw strings (not wrapped in markdown link syntax that could break paste-download).
- [ ] **Sponsors** mentioned in break/meal descriptions are captured.
- [ ] **Itinerary events** do not contain duplicate entries that also appear as presentation sessions.
- [ ] **Extraction Notes** section is reviewed for any flagged ambiguities.
