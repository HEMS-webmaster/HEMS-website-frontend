...
import { 
  getWorkshopDir, 
  getProceedingsDir, 
  getMasterWorkshopsPath, 
  getArchivesDataDir, 
  getCorporateRegistryPath, 
  getFrontendSponsorsDir 
...
export async function POST(request: Request) {
  try {
    ...
    const masterPath = getMasterWorkshopsPath();
    await fs.writeFile(masterPath, JSON.stringify(data, null, 2), 'utf8');

    const archivesDir = getArchivesDataDir();
    ...
    for (const ws of data) {
      ...
      const checkFileExists = (category: string, wsNum: number, session: string | null, fileName: string) => {
        ...
        let sessionSubdir = '';
        if (category === 'Administrative') sessionSubdir = 'Administrative';
        else if (category === 'Student_Award') sessionSubdir = 'Student_Award';
        else if (category === 'Poster') sessionSubdir = 'Posters';
        else {
          sessionSubdir = (session || 'General').replace(/\s*\(.*?\)\s*/g, '').trim().replace(/[^a-zA-Z0-9]/g, '_');
        }
        const wsDir = getWorkshopDir(wsOrdinal);
        let fullPath = path.join(wsDir, sessionSubdir, fileName);
        if (!fsSync.existsSync(fullPath)) {
          fullPath = path.join(getProceedingsDir(), wsOrdinal, sessionSubdir, fileName);
        }
      ...
      if (ws.sponsors && ws.sponsors.length > 0) {
        ...
        try {
          const regRaw = await fs.readFile(getCorporateRegistryPath(), 'utf8');
        ...
        yearData.sponsors = ws.sponsors.map((s: any) => {
          ...
          if (logoFile) {
            try {
              const logoOnDisk = path.join(getFrontendSponsorsDir(), logoFile);
      ...
      if (hostSponsor && hostSponsor.name) {
        ...
        if (hc.logo_file) {
          try {
            const hcLogoPath = path.join(getFrontendSponsorsDir(), hc.logo_file);
          ...
          if (sessionGroup.presentations) {
            for (const pres of sessionGroup.presentations) {
              sessionItem.talks.push({
                ...
                legacy_url: pres.legacy_url || pres.url || "",
                legacy_abstract_url: pres.legacy_abstract_url || pres.abstract_url || "",
      ...
      if (ws.posters && ws.posters.length > 0) {
        for (const poster of ws.posters) {
          ...
          posterSession.talks.push({
            ...
            legacy_url: poster.legacy_url || poster.url || "",
            legacy_abstract_url: poster.legacy_abstract_url || poster.abstract_url || "",
