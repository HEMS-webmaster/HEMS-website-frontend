...
export default function PresentationsManager({ presentation_sessions = [], wsNum, onChange }: PresentationsManagerProps) {
  ...
  const toggleSession = (gIdx: number) => {
    setOpenSessions(prev => ({ ...prev, [gIdx]: !prev[gIdx] }));
  };

  const isSessionOpen = (gIdx: number) => {
    return openSessions[gIdx] !== false; // Default to open
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {(presentation_sessions || []).map((group, gIdx) => {
        const isOpen = isSessionOpen(gIdx);
        return (
          <div key={gIdx} className="bg-slate-800 p-0 rounded-lg overflow-hidden border border-slate-700 shadow-md">
            {/* Collapsible Header */}
            <div className="flex justify-between items-center bg-slate-700/30 px-5 py-3 select-none border-b border-slate-700/50">
              <div className="flex items-center gap-3 cursor-pointer flex-1" onClick={() => toggleSession(gIdx)}>
                <span className="text-slate-400 text-sm transition-transform duration-200">
                  {isOpen ? '▼' : '▶'}
                </span>
                <span className="font-bold text-sky-400">
                  {group.title || `Session ${gIdx + 1}`} {group.date ? `(${group.date})` : ''} {group.location ? `@ ${group.location}` : ''}
                </span>
              </div>
              <div className="flex items-center gap-2">
                {sessionDownloadStatus[gIdx]?.running ? (
                  <span className="text-[11px] text-yellow-400 font-bold font-mono animate-pulse mr-2">
                    ⟳ {sessionDownloadStatus[gIdx].done}/{sessionDownloadStatus[gIdx].total}
                  </span>
                ) : sessionDownloadStatus[gIdx] ? (
                  <span className="text-[11px] text-green-400 font-bold font-mono mr-2">
                ...
                <button
                  ...
                  className="bg-amber-700/40 hover:bg-amber-700/60 disabled:opacity-40 disabled:cursor-not-allowed text-amber-200 px-2 py-0.5 rounded text-[11px] font-bold transition-colors"
                ...
                <button 
                  ...
                  className="text-red-400 hover:text-red-300 font-bold px-2 text-sm ml-2"
            ...
            {isOpen && (
              <div className="p-5 bg-slate-800/40 space-y-4">
